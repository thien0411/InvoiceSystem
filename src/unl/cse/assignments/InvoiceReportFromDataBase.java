...
public class InvoiceReportFromDataBase {
	 int i = 0;
	...
	private String generateSummaryReport() {
		...
		for(int z =0; z< DataBean.invoiceList.size(); z++){
			String salePerson = null;
			Invoice invoice =  DataBean.invoiceList.get(z);
			//invoice.calculating();
	...
	private String getTravelSummary(Invoice invoice) {
		...
		for(int z =0 ; z< invoice.getListOfTickets().size(); z++ ){
			...
			try
			{
				...
				sb.append(String.format("%48s %30s", "("+ticket.getDepartureAirport().getAirportCode()+") "+date1.format(date2),
		...
		for(int z =0 ; z< invoice.getListOfService().size(); z++){
			...
			if (temp.equals("SR")){
				sb.append(String.format("%-10s %-82s %s %-13.2f %s %-11.2f %s %.2f\n",code,serviceName+" (" +quantity + " units @ " + "$" +
										invoice.getListOfService().get(z).getCost() + "/units "+"5% off)","$",servicePrice,"$",taxes,"$",total));
			}
			if (temp.equals("SC")){
				sb.append(String.format("%-10s %-82s %s %-13.2f %s %-11.2f %s %.2f\n"
	...
	for ( i = 0; i < DataBean.invoiceList.size(); i++){
		...
		sb.append("------------------------------------------------------------------------------------------------------------\n");
		sb.append(getTravelSummary(DataBean.invoiceList.get(i)));
		sb.append(getCustomerSummary(DataBean.invoiceList.get(i)));
		sb.append(getCostSummary(DataBean.invoiceList.get(i)));
	}
	...
	public static void main(String args[]) {
		...
		InvoiceReportFromDataBase ir = new InvoiceReportFromDataBase();
		String summary = ir.generateSummaryReport();
		String details = ir.generateDetailReport(DataBean.invoiceList);
